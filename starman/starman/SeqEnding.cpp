...
//    BGM::get_ton()->play(10);

    {
        ...
        auto it = std::find(finishQuest.begin(), finishQuest.end(), "Q81");
        ...
        {
            {
                ...
                vs.clear();
                vs.push_back("ホシマンを見た通行人に通報され、すぐにホシマンは警察に保護された。");
                vs.push_back("ホシマンは警察に事情を話し、無人島へ救助隊が派遣されることになった。");
                vvs.push_back(vs);

                vs.clear();
                vs.push_back("救助隊は生存者を一人だけ発見した。");
                vvs.push_back(vs);

                vs.clear();
                vs.push_back("救助隊は生存者に、ほかの生存者はいないか尋ねた。");
                vvs.push_back(vs);
