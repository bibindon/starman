...
void SeqBattle::OperateMenu(eSequence* sequence)
{
    ...
    if (result == "ç≈èâÇ©ÇÁ")
    {
        // TODO ç≈èâÇ©ÇÁÅAÇÕñ≥Ç≠ÇµÇƒÇ‡ó«Ç¢Ç©Ç‡ÇµÇÍÇ»Ç¢ÅB
//        m_bShowMenu = false;
//        Camera::SleepModeOFF();
//        Common::SetCursorVisibility(false);
//        *sequence = eSequence::OPENING;
    ...
    {
        // TODO ÉJÉÅÉâÇê¶Ç≠âìÇ¥ÇØÇƒÉ^ÉCÉgÉãâÊñ Ç…Ç∑ÇÈ
...
void SeqBattle::OperateTitle()
{
    ...
    {
        // ÉçÅ[ÉfÉBÉìÉOíÜÅAÉJÉÅÉâà⁄ìÆíÜÅAÉtÉFÅ[ÉhÉAÉEÉgíÜÇÕëÄçÏïsî\Ç…Ç∑ÇÈ
        ...
        else
        {
            ...
            {
                // ÉJÉÅÉâÇÉvÉåÉCÉÑÅ[ÇÃà íuÇ…å¸Ç©Ç¡Çƒ60ÉtÉåÅ[ÉÄÇ≈ìûíBÇ∑ÇÈÇÊÇ§Ç…à⁄ìÆÇ≥ÇπÇÈÅB
            ...
            {
                // ë¶ç¿Ç…ÉIÅ[ÉvÉjÉìÉOÇ™énÇ‹ÇÈÇÃÇ≈ÇÕÇ»Ç≠ÅA
                // ÉtÉFÅ[ÉhÉAÉEÉgÇï`âÊÇµÅA
                // ÉtÉFÅ[ÉhÉAÉEÉgÇ™äÆóπÇµÇΩÇÁÉIÅ[ÉvÉjÉìÉOÇ™énÇ‹ÇÈÇÊÇ§Ç…Ç∑ÇÈÅB
...
{
    // 1ÉtÉåÅ[ÉÄñ⁄ÇÕÅAOperateTitleä÷êîÇ™åƒÇŒÇÍÇÈëOÇ…åƒÇŒÇÍÇÈÇ±Ç∆Ç…íçà”Ç∑ÇÈ
    ...
    if (m_titleThread != nullptr && m_titleLoaded.load() == false)
    {
        ...
        // âEï˚å¸Ç…8ÉsÉNÉZÉãÅAâ∫ï˚å¸Ç…31ÉsÉNÉZÉãà⁄ìÆÇ∑ÇÍÇŒÉNÉâÉCÉAÉìÉgóÃàÊÅH
        // ñ{ìñÇÕÇøÇ·ÇÒÇ∆Ç‚ÇÁÇ»Ç¢Ç∆Ç¢ÇØÇ»Ç¢ÅB
...
void SeqBattle::UpdatePerSecond()
{
    ...
    {
        dateTime->IncreaseDateTime(0, 0, 1, 10, 0); // 1ïbÇ≈1éûä‘Ç∆Ç©åoâﬂÇ≥ÇπÇΩÇ¢éûóp
    }
