...
void SeqBattle::OperateStorehouse()
{
    ...
    if (KeyBoard::IsDownFirstFrame(DIK_RETURN))
    {
        ...
        if (vs.at(0) == "left")
        {
            ...
            // ëïîıíÜÇÃë‹ÇæÇ¡ÇΩÇÁçÌèúÇ≈Ç´Ç»Ç¢ÇÊÇ§Ç…Ç∑ÇÈ
            {
                ...
                for (auto it = allBag.begin(); it != allBag.end(); ++it)
                {
                    ...
                    {
                        PopUp2::Get()->SetText("ëïîıíÜÇÃë‹Çëqå…Ç…à⁄ìÆÇ∑ÇÈÇ±Ç∆ÇÕÇ≈Ç´Ç»Ç¢");
                        equipBagExist = true;
                        break;
                    }
                }
            ...
            // ëïîıíÜÇÃïêäÌÇæÇ¡ÇΩÇÁçÌèúÇ≈Ç´Ç»Ç¢ÇÊÇ§Ç…Ç∑ÇÈ
            {
                auto weapon = Common::Status()->GetEquipWeapon();
                if (weapon.GetId() == id_ && weapon.GetSubId() == subId_)
                {
                    PopUp2::Get()->SetText("ëïîıíÜÇÃïêäÌÇëqå…Ç…à⁄ìÆÇ∑ÇÈÇ±Ç∆ÇÕÇ≈Ç´Ç»Ç¢");
...
void SeqBattle::OperateCommand()
{
    ...
    {
        // êŒÇ©êŒïÄÇëïîıÇµÇƒÇ¢Ç»Ç¢Ç∆êÿÇÍÇ»Ç¢
        auto status = NSStarmanLib::StatusManager::GetObj();
        auto itemInfo = status->GetEquipWeapon();
        auto name = itemInfo.GetItemDef().GetName();
        if (name != "êŒïÄ" && name != "ècí∑ÇÃêŒ")
        {
            PopUp2::Get()->SetText("ìKêÿÇ»ìπãÔÇëïîıÇµÅAÉXÉ^É~ÉiìôÇ™Ç†ÇÈíˆìxÇ»Ç¢Ç∆î∞çÃÇäJénÇ≈Ç´Ç»Ç¢");
        }
        else
        {
    ...
    else if (result == "çÃéÊ")
    {
        ...
        if (stamina <= 16.f)
        {
            PopUp2::Get()->SetText("ÉXÉ^É~ÉiÇ™Ç†ÇÈíˆìxÇ»Ç¢Ç∆çÃéÊÇäJénÇ≈Ç´Ç»Ç¢");
        }
        else
        {
            leave = true;
    ...
    else if (result == "îøÇí£ÇÈ")
    {
        auto voyage = NSStarmanLib::Voyage::Get();
        voyage->SetSailCurrentRaft(true);
    }
    else if (result == "îøÇèÙÇﬁ")
    {
        auto voyage = NSStarmanLib::Voyage::Get();
        voyage->SetSailCurrentRaft(false);
    }
    else if (result == "åªç›ÇÃï˚äpÇ…ÇRéûä‘ëÜÇÆ")
    {
        auto voyage = NSStarmanLib::Voyage::Get();
        voyage->Set3HoursAuto();
    }
    else if (result == "óßÇøè„Ç™ÇÈ")
    {
        auto voyage = NSStarmanLib::Voyage::Get();
        voyage->SetRaftMode(false);
    }
    else if (result == "ÉCÉJÉ_Ç…èÊÇÈ")
    {
        auto voyage = NSStarmanLib::Voyage::Get();

        // ë‹ÇëïîıÇµÇƒÇ¢ÇΩÇÁÉCÉJÉ_Ç…èÊÇÈÇ±Ç∆ÇÕÇ≈Ç´Ç»Ç¢
        ...
        {
            PopUp2::Get()->SetText("ë‹ÇëïîıÇµÇƒÇ¢ÇΩÇÁÉCÉJÉ_Ç…èÊÇÈÇ±Ç∆ÇÕÇ≈Ç´Ç»Ç¢");
        }
    }
    else if (result == "ÉCÉJÉ_ÇÃë‹Çå©ÇÈ")
    {
        // TODO
    }

    // ÉRÉ}ÉìÉhâÊñ Çï¬Ç∂ÇÈèÍçáÅAíEèoÉRÉ}ÉìÉhÇÕçÌèúÇ∑ÇÈ
    if (leave)
    {
        Camera::SetCameraMode(eCameraMode::BATTLE);
        Common::SetCursorVisibility(false);
...
void SeqBattle::OperateSleep()
{
    if (m_fadeBlackCount == 1)
    {
        ...
        {
            PopUp2::Get()->SetText("âäñÇñ@ÇÃÉåÉxÉãÇ™è„Ç™Ç¡ÇΩ");
        }

        if (status->GetLevelUpIce())
        {
            PopUp2::Get()->SetText("ïXñÇñ@ÇÃÉåÉxÉãÇ™è„Ç™Ç¡ÇΩ");
        }

        if (status->GetLevelUpDark())
        {
            PopUp2::Get()->SetText("à≈ñÇñ@ÇÃÉåÉxÉãÇ™è„Ç™Ç¡ÇΩ");
        }

        if (status->GetLevelDownFire())
        {
            PopUp2::Get()->SetText("âäñÇñ@ÇÃÉåÉxÉãÇ™â∫Ç™Ç¡ÇΩ");
        }

        if (status->GetLevelDownIce())
        {
            PopUp2::Get()->SetText("ïXñÇñ@ÇÃÉåÉxÉãÇ™â∫Ç™Ç¡ÇΩ");
        }

        if (status->GetLevelDownDark())
        {
            PopUp2::Get()->SetText("à≈ñÇñ@ÇÃÉåÉxÉãÇ™â∫Ç™Ç¡ÇΩ");
        }
    }
...
void SeqBattle::OperatePickPlant()
{
    if (m_eFadeSeq == eFadeSeq::Finish)
    {
        ...
        // TODO äÆëSÉâÉìÉ_ÉÄÇ≈ÇÕÇ»Ç≠ÅAèÍèäÇ∆å©ÇΩñ⁄Ç…ÇÊÇ¡ÇƒëΩè≠ÇÃÇŒÇÁÇ¬Ç´Ç™Ç†Ç¡ÇƒÇŸÇµÇ¢
        if (rand % 100 < 10)
        {
            pick = "ì‰ÇÃëêÇP";
        }
        ...
        PopUp2::Get()->SetText(pick + "ÇéËÇ…ì¸ÇÍÇΩÅB");
        ...
        // ëÃóÕÇè¡îÔ
        // ëïîıïêäÌÇ≈è¡îÔÇ∑ÇÈéûä‘Ç‚ëÃóÕÇ™ïœÇÌÇÈ
        Common::Status()->PickPlant();

        // ÉAÉCÉeÉÄÇÉCÉìÉxÉìÉgÉäÇ…í«â¡
...
void SeqBattle::OperateCutTree()
{
    if (m_eFadeSeq == eFadeSeq::Finish)
    {
        PopUp2::Get()->SetText("ç◊Ç¢ñÿÇÃä≤ÇéËÇ…ì¸ÇÍÇΩÅB");
        ...
        else if (name == "êŒïÄ")
        {
            ...
            // éûä‘ÇåoâﬂÇ≥ÇπÅAêŒïÄÇÃëœãvílÇå∏ÇÁÇ∑
            if (level == 0 || level == -1)
            {
        ...
        // ÉAÉCÉeÉÄÇÉCÉìÉxÉìÉgÉäÇ…í«â¡
        auto itemDef = Common::ItemManager()->GetItemDef("ç◊Ç¢ñÿÇÃä≤");
        Common::Inventory()->AddItem(itemDef.GetId());
...
void SeqBattle::OperateVoyage()
{
    ...
    {
        m_commandManager.Upsert("óßÇøè„Ç™ÇÈ", true);
        m_commandManager.Upsert("ÇRéûä‘ëÜÇÆ", true);
    }
}
...
void SeqBattle::Confirm(eSequence* sequence)
{
    ...
    else if (m_bTalkable)
    {
        ...
        if (npcName.empty() == false)
        {
            ...
            // âÔòbÇµÇΩÇÃÇ≈âÔòbâ¬î\èÛë‘ÇâèúÇ∑ÇÈ
...
void SeqBattle::RenderCommon2D()
{
    ...
    if (m_eState != eBattleState::LOAD &&
        m_eState != eBattleState::TITLE &&
        // m_eState != eBattleState::MENU && // ÉÅÉjÉÖÅ[âÊñ Ç≈Ç‡É|ÉbÉvÉAÉbÉvÇ™ïKóv
...
void SeqBattle::UpdatePerSecond()
{
    ...
    if (dead)
    {
        if (m_eState != eBattleState::DEAD)
        {
            ...
            {
                PopUp2::Get()->SetText("éÄñSÅBÉèÅ[ÉhÉuÉåÉXÇà˘ÇÒÇæèÍèäÇ≈ïúäàÇµÇ‹Ç∑ÅB");
            }
        }
        return;
    }
    ...
    //-------------------------------------
    // ç≈ëÂêœç⁄ó Çí¥Ç¶ÇƒÇ¢ÇΩÇÁÉÅÉbÉZÅ[ÉWÇï\é¶
    //-------------------------------------
    {
            ...
            previousVolMax != volMax)
        {
            if (vol > volMax)
            {
                PopUp2::Get()->SetText("ç≈ëÂêœç⁄ó Çí¥Ç¶ÇƒÇ¢Ç‹Ç∑ÅB");
            }
        }
        previousVol = vol;
        previousVolMax = volMax;
...
void SeqBattle::OperateNormal(eSequence* sequence)
{
    ...
    //-------------------------------------------------
    {
        ...
        if (work == 0)
        {
            // ãﬂÇ≠Ç…É`ÉFÉXÉgÇ™Ç†Ç¡ÇΩÇÁëqå…ã@î\Ç™égÇ¶ÇÈÅB
            {
        ...
        else if (work == 5)
        {
            // ãﬂÇ≠Ç…êAï®Ç™Ç†ÇÈÇ©
            {
                ...
                {
                    m_commandManager.Upsert("çÃéÊ", true);
                }
                else
                {
                    m_commandManager.Upsert("çÃéÊ", false);
                }
            }
        }
        else if (work == 10)
        {
            // ãﬂÇ≠Ç…ñÿÇ™Ç†ÇÈÇ©
            {
                ...
                {
                    m_commandManager.Upsert("î∞çÃ", true);
                }
                else
                {
                    m_commandManager.Upsert("î∞çÃ", false);
                }
            }
        }
    }
