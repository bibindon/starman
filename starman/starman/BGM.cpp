        ...
        for (auto& envBgm : envBgmMap)
        {
            if (envBgm.second.m_bChanged)
            {
                if (envBgm.second.m_bPlay)
                {
                    ...
                    // å¯â âπÇ…ëŒÇµÇƒÉtÉFÅ[ÉhÉCÉìÇÇ‚ÇÈÇ∆BGMÇ™ñ¬ÇÁÇ»Ç≠Ç»Ç¡ÇƒÇµÇ‹Ç§ÅB
                    // åªèÛç¢Ç¡ÇƒÇ¢Ç»Ç¢ÇÃÇ≈ï˙íu
                    play(envBgm.second.m_filename, envBgm.second.m_volume, false);
                }
                else
                {
                    // çƒê∂Ç∆í‚é~ÇìØéûÇ…çsÇÌÇÍÇÈÇ∆àÍìxÇ‡playä÷êîÇ™åƒÇŒÇÍÇ∏Ç…stopä÷êîÇ™åƒÇŒÇÍÇÈÇ±Ç∆Ç™Ç†ÇÈÅB
